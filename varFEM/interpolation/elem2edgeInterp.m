function [elemuhM,elemuhP,elemnx,elemny] = elem2edgeInterp(wStr,Th,uh,Vh,quadOrder)
...
if nargout==4
    ne = Th.ne;
    rep = kron([1 2 3], ones(1,ng));
    id3 = elem2edge(:, rep);
    nex = ne(:,1); ney = ne(:,2);
    elemnx = nex(id3).*sgnelem(:,rep);
    elemny = ney(id3).*sgnelem(:,rep);
end